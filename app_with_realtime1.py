...
def main():
    ...
    if page == "📊 Individual Stock Analysis":
        ...
        st.markdown("Select a stock and a target date — the model trains on all data before that date and predicts the closing price for that day.")
        st.markdown("---")
        ...
        with col2:
            if st.session_state.get("prediction_result"):
                ...
                <p><small>Model: ARIMA{result["arima_order"]} | AIC: {result["aic"]:.2f}</small></p>
                </div>
                """, unsafe_allow_html=True)
